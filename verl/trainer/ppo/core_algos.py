...
@register_policy_loss("low_entropy_kl_cov")
def compute_policy_loss_low_entropy_kl_cov(
    old_log_prob,
    log_prob,
    advantages,
    response_mask,
    ...
    flat_entropy = entropy[response_mask.bool()]  # shape: (num_valid_tokens,)

    k = max(1, int(flat_entropy.shape[0] * 0.8))  
    low_entropy_vals, low_entropy_indices = torch.topk(flat_entropy, k, largest=False)

    new_mask = torch.zeros_like(response_mask, dtype=torch.bool).view(-1)
    flat_mask = response_mask.bool().view(-1)
    valid_indices = flat_mask.nonzero(as_tuple=False).squeeze(-1)
    selected_indices = valid_indices[low_entropy_indices]
    new_mask[selected_indices] = True
    response_mask = new_mask.view_as(response_mask)

    pg_losses[response_mask] = pg_losses_kl[response_mask]
    
    pg_loss = agg_loss(loss_mat=pg_losses, loss_mask=response_mask, loss_agg_mode=loss_agg_mode)

    return pg_loss, torch.tensor(0.0), ppo_kl_abs, torch.tensor(0.0)

@register_policy_loss("low_entropy_kl_cov_v2")
def compute_policy_loss_low_entropy_kl_cov_v2(
    old_log_prob,
    log_prob,
    advantages,
    response_mask,
    ...
    """
    Low-entropy token KL with covariance-aware scaling (v2).
    """
    kl_cov_ratio = config.policy_loss.kl_cov_ratio if config.policy_loss.kl_cov_ratio is not None else 0.0002
    ppo_kl_coef = config.policy_loss.ppo_kl_coef if config.policy_loss.ppo_kl_coef is not None else 1.0
    high_cov_coef = 2.0
    ...
    flat_entropy = entropy[response_mask.bool()]  # shape: (num_valid_tokens,)
    k = max(1, int(flat_entropy.shape[0] * 0.8))
    _, low_entropy_indices = torch.topk(flat_entropy, k, largest=False)

    flat_mask = response_mask.bool().view(-1)
    valid_indices = flat_mask.nonzero(as_tuple=False).squeeze(-1)
    selected_indices = valid_indices[low_entropy_indices]

    all_valid_adv = advantages[response_mask].detach().reshape(-1).cpu()
    all_valid_logp = log_prob[response_mask].detach().reshape(-1).cpu()

    low_adv = all_valid_adv[low_entropy_indices]
    low_logp = all_valid_logp[low_entropy_indices]
    cov_lst = (low_adv - low_adv.mean()) * (low_logp - low_logp.mean())

    k_percent_nums = max(1, int(len(cov_lst) * kl_cov_ratio))
    large_cov_idxs = torch.topk(cov_lst, k_percent_nums, largest=True).indices
    small_cov_idxs = torch.topk(cov_lst, k_percent_nums, largest=False).indices

    pg_losses_kl_high = -advantages * ratio + (ppo_kl_coef * high_cov_coef) * abs_kl
    pg_losses_kl_low  = -advantages * ratio + (ppo_kl_coef * low_cov_coef) * abs_kl

    if len(large_cov_idxs) > 0:
        sel_idx = selected_indices[large_cov_idxs]
        pg_losses.view(-1)[sel_idx] = pg_losses_kl_high.view(-1)[sel_idx]

    if len(small_cov_idxs) > 0:
        sel_idx = selected_indices[small_cov_idxs]
        pg_losses.view(-1)[sel_idx] = pg_losses_kl_low.view(-1)[sel_idx]
